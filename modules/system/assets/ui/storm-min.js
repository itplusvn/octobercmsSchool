...
FilterWidget.prototype.addItemsToListElement=function($ul,items){$.each(items,function(key,obj){var item=$('<li />').data({'item-id':obj.id}).append($('<a />').prop({'href':'js:;',}).text(obj.name))
$ul.append(item)})}
...
processedTitle=this.generateTitleText(title,-1),$link=$('<a/>').attr('href','js:;').text(processedTitle),$li=$('<li/>'),$pane=$('<div>').html(content).addClass('tab-pane');$link.attr('title',title)
$li.append($link)
